...
test.describe('Siparişler Sayfası', () => {
  ...
  test('siparişler sayfası yükleniyor', async ({ page }) => {
    ...
    await page.waitForTimeout(3000); // Sayfanın yüklenmesi için bekle
    
    // Siparişler sayfasının yüklendiğini kontrol et
    await expect(page).toHaveURL(/\/orders/, { timeout: 10000 });
    
    // Siparişler başlığının görünür olduğunu kontrol et - daha esnek
    const hasOrdersHeading = await page.getByRole('heading', { name: /Siparişler|sipariş|Orders/i }).first().isVisible({ timeout: 5000 }).catch(() => false);
    const hasNoOrders = await page.getByText(/sipariş bulunamadı|henüz sipariş|no orders|orders/i).first().isVisible({ timeout: 5000 }).catch(() => false);
